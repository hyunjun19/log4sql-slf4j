 ...
 * http://log4sql.sourceforge.net version : 1.0 Date Time: 2007. 9. 12 오후
...
public class SL {
	...
	Logger logger = LoggerFactory.getLogger(SL.class);
	...
	private String now() {
		return (new java.text.SimpleDateFormat(this.dateFormat == null ? DEFAULT_DATE_FORMAT : dateFormat)).format(new java.util.Date());
	}
	...
	public void logError(Throwable e) {
		try {
			if (checkLogLevel(getError()))
				systemLogging(
						new PositionInfo(e.getCause() != null && e.getCause().getStackTrace().length >= ERROR_LOG_INDEX ? e.getCause().getStackTrace()[ERROR_LOG_INDEX]
								: e.getStackTrace().length > 0 ? e.getStackTrace()[e.getStackTrace().length - 1] : e.getStackTrace()[0], e.getCause() != null
								&& e.getCause().getStackTrace().length >= ERROR_LOG_INDEX ? e.getCause().getStackTrace() : e.getStackTrace(), e.getCause() != null
								&& e.getCause().getStackTrace().length >= ERROR_LOG_INDEX ? e.getCause().getMessage() : e.getMessage()), e.getCause() != null ? e.getCause()
		...
		try {
			if (checkLogLevel(getError()))
				systemLogging(
						new PositionInfo(e.getCause() != null && e.getCause().getStackTrace().length >= ERROR_LOG_INDEX ? e.getCause().getStackTrace()[ERROR_LOG_INDEX]
								: e.getStackTrace().length > 0 ? e.getStackTrace()[e.getStackTrace().length - 1] : e.getStackTrace()[0], e.getCause() != null
								&& e.getCause().getStackTrace().length > 0 ? e.getCause().getStackTrace() : e.getStackTrace(), e.getCause() != null
								&& e.getCause().getStackTrace().length > 0 ? e.getCause().getMessage() : e.getMessage()), e.getCause() != null ? e.getCause().getClass().getName()
	...
	private void systemLogging(PositionInfo positionInfo, String exceptionClassName) {
		...
		sb.append("『").append(positionInfo.getClassName()).append(":").append(positionInfo.getMethodName()).append("(").append(positionInfo.getLineNumber()).append(")』\n");
	...
	private void systemLoggingForPositionCheck(PositionInfo positionInfo, String message) {
		outter(new StringBuffer().append("[Position Check Start").append(message != null ? " :\n" + message : "").append("]\n").append(positionInfo.getMsg()).append("\n")
